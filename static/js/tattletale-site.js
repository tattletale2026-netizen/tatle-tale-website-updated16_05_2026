...
(function () {
    ...
    function buildNavbar() {
        document.body.classList.add('tt-unified-nav-active');
...
/* FINAL UNIFIED LOGO POSITION PATCH START */
(function () {
    function injectLogoStyles() {
        if (document.getElementById('tt-unified-logo-style')) return;
        const style = document.createElement('style');
        style.id = 'tt-unified-logo-style';
        style.textContent = `
            .tt-unified-logo-link {
            ...
            body.tt-unified-logo-active .logo-wrapper:not(.tt-unified-logo-wrapper),
            body.tt-unified-logo-active #SITE_HEADER img[alt="Homepage"] {
    ...
    function buildUnifiedLogo() {
        injectLogoStyles();
        document.body.classList.add('tt-unified-logo-active');
        let logo = document.querySelector('.tt-unified-logo-link');
        if (!logo) {
            logo = document.createElement('a');
            logo.className = 'tt-unified-logo-link tt-unified-logo-wrapper';
            logo.href = '/';
            logo.setAttribute('aria-label', 'Tatle tale home');
            logo.innerHTML = '<img src="/static/images/logo.png" alt="Tatle tale Logo">';
            const navWrap = document.querySelector('.tt-unified-navbar-wrap');
            if (navWrap && navWrap.parentNode) {
                navWrap.parentNode.insertBefore(logo, navWrap.nextSibling);
            } else {
    ...
    document.addEventListener('DOMContentLoaded', buildUnifiedLogo);
    window.addEventListener('load', buildUnifiedLogo);
    setTimeout(buildUnifiedLogo, 300);
    setTimeout(buildUnifiedLogo, 1000);
    setTimeout(buildUnifiedLogo, 2000);
})();
/* FINAL UNIFIED LOGO POSITION PATCH END */
