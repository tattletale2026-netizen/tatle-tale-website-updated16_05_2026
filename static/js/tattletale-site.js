...
function fixNavbarLinks() {
    ...
    links.forEach(link => {
        ...
        if (href.includes('about-lana')) link.setAttribute('href', '/about_us');
...
(function () {
  ...
  function standardizeNav(nav) {
    ...
    const keyOrder = ['home','story','workshops','survey','about','contact'];
    const labelByKey = {
      home: 'Home', story: 'Tatle tale Story', workshops: 'Workshops', survey: 'Community Survey', about: 'About Us', contact: 'Contact'
    };
    const hrefByKey = {
      home: '/', story: '/turtle-tales-story', workshops: '/workshop-chapters', survey: '/survey', about: '/about_us', contact: '/contact'
    };
...
(function () {
    ...
    function buildNavbar() {
        if (document.body.classList.contains('tt-use-page-header-nav')) {
            document.querySelectorAll('.tt-unified-navbar-wrap').forEach(el => el.remove());
            return;
        }
        ...
        document.body.classList.add('tt-has-unified-fixed-nav');
    ...
    function looksLikeMainNav(el) {
        ...
        const hits = ['home', 'tatle tale story', 'workshops', 'community survey',  'about us', 'contact']
            .filter(label => text.includes(label)).length;
        return hits >= 3;
    }

    function hideOldNavbars() {
        if (document.body.classList.contains('tt-use-page-header-nav')) return;
        const selectors = [
            'nav',
            '.site-nav',
            '.tt-mobile-menu-nav',
...
/* TOP-LEFT LOGO PATCH START */
(function () {
    function addTopLeftLogo() {
        if (document.body.classList.contains('tt-use-page-header-nav')) return;
        if (document.querySelector('.tt-top-left-logo')) return;
        const logo = document.createElement('img');
        logo.className = 'tt-top-left-logo';
        logo.src = '/static/images/tatle-tale-top-left-logo.png';
        logo.alt = 'Tatle Tale logo';
        document.body.appendChild(logo);
    }

    document.addEventListener('DOMContentLoaded', addTopLeftLogo);
    window.addEventListener('load', addTopLeftLogo);
    setTimeout(addTopLeftLogo, 300);
})();
/* TOP-LEFT LOGO PATCH END */


/* REMOVE BOOKINGS NAVBAR ITEM START */
(function () {
    function isBookingsLink(a) {
        const text = (a.innerText || a.textContent || '').replace(/\s+/g, ' ').trim().toLowerCase();
        const href = (a.getAttribute('href') || '').toLowerCase();
        return text === 'bookings' || href.includes('/event-booking') || href.includes('/bookings');
    }

    function removeBookingsNavbarItem() {
        document.querySelectorAll('a').forEach(a => {
            if (!isBookingsLink(a)) return;

            const mainNav = a.closest(
                '.tt-unified-navbar, .tt-unified-navbar-wrap, nav, [role="navigation"], .wixui-horizontal-menu, .wixui-menu, [data-testid*="menu"], .menu, header'
            );
            ...
            const item =
                a.closest('li') ||
                a.closest('[data-part="menu-item"]') ||
                a.closest('[role="menuitem"]') ||
                a.closest('[data-testid*="item"]') ||
                a;
    ...
    document.addEventListener('DOMContentLoaded', removeBookingsNavbarItem);
    window.addEventListener('load', removeBookingsNavbarItem);
    setTimeout(removeBookingsNavbarItem, 100);
    setTimeout(removeBookingsNavbarItem, 500);
    setTimeout(removeBookingsNavbarItem, 1500);
    setTimeout(removeBookingsNavbarItem, 3000);

    const observer = new MutationObserver(removeBookingsNavbarItem);
    observer.observe(document.documentElement, { childList: true, subtree: true });
})();
/* REMOVE BOOKINGS NAVBAR ITEM END */
